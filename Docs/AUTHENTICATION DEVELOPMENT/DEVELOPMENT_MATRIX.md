...
| 32 | Privileged Administrative Break Glass Sensitive Operation And Reauthentication | Parcial | `FreshAuthenticationRequiredException.php`, `AuthExceptionMapper.php`, `AuthManager` exige freshness configurable para revocacion remota de sesiones y `QuantumExceptionHandlerTest.php` / `AuthManagerTest.php` validan la semantica HTTP y el bloqueo de operaciones sensibles | Faltan flows privilegiados, break-glass, proofs de reauth y reautenticacion reforzada mas general |
...
| 42 | Privacy Data Minimization Retention Consent And Security Metadata Governance | Parcial | `AuthManager` reduce `User-Agent` a `client_family`, `client_platform`, `device_kind` y `device_reference` pseudonimizado, reduce IP a `ip_prefix`, evita exponer secretos bearer en inventory, persiste solo `credential_hash` del trusted-device cookie y actualiza `last_activity_at` server-side; `config/auth.php` define `auth.session.cleanup.tombstone_retention` y reglas basicas de `trusted_devices`; `AuthManagerTest.php` y repositorios validan minimizacion, emision de cookies y retencion minima | Faltan politicas de retencion mas ricas, cleanup gobernado, rotacion de credenciales y gobierno explicito de metadata sensible |
