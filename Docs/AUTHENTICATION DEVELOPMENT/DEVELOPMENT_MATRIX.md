...
| 22 | Login Logout Sign In Sign Out Entry Point And User Authentication Flow | Parcial | `AuthManager` soporta `attempt()`, `attemptOrFail()`, `stepUp()`, `stepUpOrFail()`, `login()`, `logout()`, `setUser()`, `context()`, `currentSession()`, `sessions()`, `revokeSession()` y `revokeOtherSessions()`; facade `Auth`; `Quantum/Middlewares/AuthMiddleware.php`; `Quantum/Middlewares/GuestMiddleware.php`; `Quantum/Middlewares/MfaMiddleware.php`; `AuthManagerTest.php` cubre login valido, invalido, policy, restauracion, expiracion, rotacion, revocacion, logout, inventory seguro, revocacion por `session_public_id`, ruta protegida, ruta guest-only, denial stale-session, assurance insuficiente por metadata, assurance explicito en contexto, login MFA local, `step_up` y entry point MFA | Faltan entry points adicionales, ownership/policy mas rica y sign-in/sign-out governance mas completa |
...
| 35 | Session Device Credential Inventory Security Center And User Security Management | Parcial | `AuthenticationSessionPublicId.php`, `AuthenticationSessionSummary.php`, `AuthenticationContext::sessionPublicId()`, `AuthManager::currentSession()`, `AuthManager::sessions()`, `AuthManager::revokeSession()`, `AuthManager::revokeOtherSessions()` y tests feature de inventory/revocacion segura | Faltan metadata de dispositivo, ultima actividad, location approximation, ownership/policy y security center distribuido |
