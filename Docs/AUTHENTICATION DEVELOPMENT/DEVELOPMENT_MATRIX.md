...
| 21 | Device Trust Device Identity And Trusted Device Credential | Parcial | `AuthManager` deriva metadata reducida de device (`client_platform`, `device_kind`), labels visibles no confiables como `Chrome on Windows`, `device_reference` pseudonimizado y trusted-device records persistentes via `TrustedDeviceRepositoryInterface`, `InMemoryTrustedDeviceRepository.php` y `FileTrustedDeviceRepository.php`; `AuthManagerTest.php` valida alta/olvido de trusted devices con MFA | Faltan trusted-device credentials cliente duraderas, referencias mas fuertes y evaluacion real de confianza |
...
| 42 | Privacy Data Minimization Retention Consent And Security Metadata Governance | Parcial | `AuthManager` reduce `User-Agent` a `client_family`, `client_platform`, `device_kind` y `device_reference` pseudonimizado, reduce IP a `ip_prefix`, evita exponer secretos bearer en inventory y actualiza `last_activity_at` server-side; `config/auth.php` ya define `auth.session.cleanup.tombstone_retention` y reglas basicas de `trusted_devices`; `AuthManagerTest.php` y repositorios validan minimizacion y retencion minima | Faltan politicas de retencion mas ricas, cleanup gobernado y gobierno explicito de metadata sensible |
