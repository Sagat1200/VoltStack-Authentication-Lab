...
| 21 | Device Trust Device Identity And Trusted Device Credential | Parcial | `AuthManager` deriva metadata reducida de device (`client_platform`, `device_kind`) y labels visibles no confiables como `Chrome on Windows`; `AuthManagerTest.php` valida exposicion segura en inventory | Faltan device references estables, trusted device credentials y evaluacion real de confianza |
| 22 | Login Logout Sign In Sign Out Entry Point And User Authentication Flow | Parcial | `AuthManager` soporta `attempt()`, `attemptOrFail()`, `stepUp()`, `stepUpOrFail()`, `login()`, `logout()`, `setUser()`, `context()`, `currentSession()`, `sessions()`, `revokeSession()` y `revokeOtherSessions()`; facade `Auth`; `Quantum/Middlewares/AuthMiddleware.php`; `Quantum/Middlewares/GuestMiddleware.php`; `Quantum/Middlewares/MfaMiddleware.php`; `AuthManagerTest.php` cubre login valido, invalido, policy, restauracion, expiracion, rotacion, revocacion, logout, inventory seguro, revocacion por `session_public_id`, hints `can_revoke/requires_reauthentication`, fresh-auth para revocacion remota, autocierre de sesion actual, ruta protegida, ruta guest-only, denial stale-session, assurance insuficiente por metadata, assurance explicito en contexto, login MFA local, `step_up` y entry point MFA | Faltan entry points adicionales, policy/authorization mas rica y sign-in/sign-out governance mas completa |
...
| 35 | Session Device Credential Inventory Security Center And User Security Management | Parcial | `AuthenticationSessionPublicId.php`, `AuthenticationSessionSummary.php`, `AuthenticationContext::sessionPublicId()` / `freshAuthenticationAt()`, `AuthManager::currentSession()`, `AuthManager::sessions()`, `AuthManager::revokeSession()`, `AuthManager::revokeOtherSessions()`, metadata reducida `client_family/client_platform/device_kind/ip_prefix/label/last_activity_at`, hints `can_revoke/requires_reauthentication`, `touch()` de recovery, freshness configurable para revocacion remota y tests feature de inventory/revocacion segura | Faltan metadata de dispositivo mas estable, authorization/policy mas rica, fresh-auth generalizado y security center distribuido |
...
| 42 | Privacy Data Minimization Retention Consent And Security Metadata Governance | Parcial | `AuthManager` reduce `User-Agent` a `client_family`, `client_platform` y `device_kind`, reduce IP a `ip_prefix`, evita exponer secretos bearer en inventory y actualiza `last_activity_at` server-side; `config/auth.php` ya define `auth.session.cleanup.tombstone_retention`; `AuthManagerTest.php` y repositorios validan minimizacion y retencion minima | Faltan politicas de retencion mas ricas, cleanup gobernado y gobierno explicito de metadata sensible |
